...
class manager:
    ...
    def add_brushes(self, *brushes):
        ...
        buffer_map = self.abstract_buffer_map["vertex"]["brush"]
        vertex_writes = {} # {(start, length): data}
        index_bundles = {} # brush: indices + offset
        for gap in self.find_gaps(buffer="vertex"):
            ...
            while gap_sufficient and len(brushes) > 0:
                ...
                brush_vertices_size = len(brush.vertices) * 44 # bytes per vert
                ...
                vertex_bundle.append(brush.vertices)
                # ^ [(position, normal, uv, colour), ...]
                new_indices = [i + (gap_start // 44) for i in brush.indices]
                index_bundles[brush] = new_indices
        ...
        vertex_writes = {(k[0], len(v) * 44): v for k, v in vertex_writes.items()}
        ...
        for gap in self.find_gaps(buffer="index", preferred_type="displacement"):
            gap_sufficient == True:
